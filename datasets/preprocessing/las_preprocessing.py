...
class LASPreprocessing(BasePreprocessing):
    ...
    def process_file(self, filepath, mode):
        ...
        if mode in ["validation", "test"]:
            ...
            if mode == "validation": #overwrite subplot 0 with the full plot
                new_instance_ids = np.unique(points[:, -1], return_inverse=True)[1]
                gt_data = (points[:, -2]) * 1000 + new_instance_ids
                processed_gt_filepath = self.save_dir / "instance_gt" / mode / f"{filebase['scene'].replace('.las', '')}_0.txt"
                np.savetxt(processed_gt_filepath, gt_data.astype(np.int32), fmt="%d")

                processed_filepath = self.save_dir / mode / f"{filebase['scene'].replace('.las', '')}_0.npy"
                np.save(processed_filepath, points.astype(np.float32))
